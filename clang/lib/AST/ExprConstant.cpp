...
bool IntExprEvaluator::VisitBuiltinCallExpr(const CallExpr *E,
  ...
  case Builtin::BI__builtin_f2_gemv:{
    APSInt Vector;
    if (!EvaluateInteger(E->getArg(0), Vector, Info))
      return false;
    auto VectorValue = Vector.getZExtValue();
    uint32_t Result = 0;
    for (auto i = 1u; i < E->getNumArgs(); i++, VectorValue>>=1) {
      APSInt MatrixCol;
      if (!EvaluateInteger(E->getArg(i), MatrixCol, Info))
        return false;
  ...
  case Builtin::BI__builtin_swizzle_solve: {
    APSInt RowIndex, Bits;
    if (!EvaluateInteger(E->getArg(0), RowIndex, Info))
      return false;
